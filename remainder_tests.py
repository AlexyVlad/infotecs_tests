...
class WebCalculatorRemainderTests(unittest.TestCase):
    url = f'http://localhost:{port}/api/remainder'
    ...
    def test_positive_numbers(self):
        ...
        self.assertEqual(result, 0, "Некорректный результат остатка от деления")
        ...
        self.assertTrue("result" in response_data, "Отсутствует ключ 'result' в ответе")
        result = response_data["result"]
        self.assertEqual(result, 5, "Некорректный результат остатка от деления")
        self.assertEqual(response_data["statusCode"], 0, "Неверный код ошибки")
        ...
        self.assertTrue("result" in response_data, "Отсутствует ключ 'result' в ответе")
        result = response_data["result"]
        self.assertEqual(result, 1, "Некорректный результат остатка от деления")
        self.assertEqual(response_data["statusCode"], 0, "Неверный код ошибки")
    ...
    def test_negative_numbers(self):
        ...
        self.assertTrue("result" in response_data, "Отсутствует ключ 'result' в ответе")
        result = response_data["result"]
        self.assertEqual(result, 2, "Некорректный результат остатка от деления")
        self.assertEqual(response_data["statusCode"], 0, "Неверный код ошибки")

        payload = {"x": -1, "y": 2147483647}
        response = requests.post(self.url, json=payload)
        self.assertEqual(response.status_code, 200, "Неверный код ответа сервера")
        response_data = response.json()
        self.assertTrue("statusCode" in response_data, "Отсутствует ключ 'statusCode' в ответе")
        self.assertTrue("result" in response_data, "Отсутствует ключ 'result' в ответе")
        result = response_data["result"]
        self.assertEqual(result, 2147483646, "Некорректный результат остатка от деления")
        self.assertEqual(response_data["statusCode"], 0, "Неверный код ошибки")
        ...
        self.assertTrue("result" in response_data, "Отсутствует ключ 'result' в ответе")
        result = response_data["result"]
        self.assertEqual(result, 3, "Некорректный результат остатка от деления")
        self.assertEqual(response_data["statusCode"], 0, "Неверный код ошибки")
